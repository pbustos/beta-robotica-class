...
/** \mainpage RoboComp::ArmController
 *
 * \section intro_sec Introduction
 *
 * The ArmController component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/ArmController --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class ArmController : public RoboComp::Application
{
public:
	ArmController (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::ArmController::run(int argc, char* argv[])
{
	...
	try
	{
		...
		jointmotor_proxy = Ice::uncheckedCast<JointMotorPrx>( communicator()->stringToProxy( proxy ) );
	}
	...
	tprx = std::make_tuple(jointmotor_proxy);
	...
	try
	{
		...
		try
		{
			...
			auto simplearm = std::make_shared<SimpleArmI>(worker);
