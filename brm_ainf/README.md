# brm_ainf
This component implements an Active Inference (AIF) agent for simultaneous localization and mapping (SLAM) in a simulated L-shaped room. It uses a Signed Distance Function (SDF) representation of the environment and Bayesian Model Reduction (BMR) to dynamically increase the complexity of its world model.
```
<YOUR BRIEFING>
```

## Dependencies
The following dependencies are required to build and run brm_ainf. Ensure they are installed and properly configured on your system before proceeding:
```
<YOUR DEPENDENCIES>
```

## Configuration parameters
Like any other component, brm_ainf requires a configuration file to start. In etc/config or etc/config.toml, you can find an example of the configuration file.

- `Lidar.Precision`: A floating-point value representing the inverse variance (precision) of the LiDAR sensor noise model. In the context of the SDF, this reflects our confidence that a measured point lies on the model's surface. A higher value means we trust the sensor more.
...
bin/brm_ainf etc/yourConfig # Execute the component
```
-----
-----
# Developer Notes
This section explains how to work with the generated code of brm_ainf, including what can be modified and how to use key features.
## Editable Files
