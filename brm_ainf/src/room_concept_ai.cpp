...
// Static initialization: limit PyTorch threads to avoid CPU overload
                ...
                const std::vector<OdometryReading> &odometry_history)
    {
        ...
        if (params.prediction_early_exit &&
            !(params.optimize_room_dimensions && !model_->use_polygon) &&
        ...
        if (!model_->use_polygon && params.optimize_room_dimensions)
        {
            param_groups.emplace_back(
                std::vector<torch::Tensor>{model_->half_extents},
        ...
        {
            optimizer.zero_grad();
            const torch::Tensor loss = loss_sdf_mse(points_tensor, *model_, params);
            loss.backward();
            ...
            // Keep room dimensions within physically plausible bounds.
            if (!model_->use_polygon && params.optimize_room_dimensions)
            {
                torch::NoGradGuard no_grad;
                const float min_hw = 0.5f * std::min(params.min_room_width, params.max_room_width);
                const float max_hw = 0.5f * std::max(params.min_room_width, params.max_room_width);
                const float min_hl = 0.5f * std::min(params.min_room_length, params.max_room_length);
                const float max_hl = 0.5f * std::max(params.min_room_length, params.max_room_length);
        ...
        {
            qWarning() << "[SDF] Non-finite SDF metric detected; keeping previous valid estimate";
            res.ok = false;
            return res;
        }
    ...
    void RoomConceptAI::try_expand_model_from_bmr(const BmrResult &bmr, float L1, float W1)
    {
        ...
        if (bmr.proposal == BmrResult::ProposalType::ADD_TWO_POINT_INDENT)
        {
            if (bmr.activated_vertex_a < 0 || bmr.activated_vertex_b < 0)
                return;

            const float a = std::clamp(std::min(bmr.indent_a, bmr.indent_b), -0.95f, 0.95f);
            const float b = std::clamp(std::max(bmr.indent_a, bmr.indent_b), -0.95f, 0.95f);
        ...
        const float L2 = std::clamp(bmr.ext_post_mean[0], 0.2f, 0.8f * L1);
        const float W2 = std::clamp(bmr.ext_post_mean[1], 0.2f, 0.8f * W1);

        // Stable wall-ID order:
        // 0 bottom, 1 right, 2 top-inner, 3 left, 4 inner step, 5 top extension.
        ...
        push_command(CmdSetPolygon{poly});
        qInfo() << "[BMR] Structural jump: L-shape activated with L2=" << L2 << " W2=" << W2;
    }
    ...
    void RoomConceptAI::run()
    {
        ...
        while (!stop_requested_.load())
        {
            ...
            // ===== 3. READ LIDAR DATA =====
            const auto& [lidar_high_] = run_ctx_.sensor_buffer->read_last();
