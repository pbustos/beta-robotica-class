    ...
    const std::vector<float>& pose_cov)
{
    ...
    // ========================================================================
    // SAMPLE K TRAJECTORIES WITH COVARIANCE-AWARE OBSTACLE COSTS
    // ========================================================================
    for (int k = 0; k < params_.K; ++k)
    {
        std::vector<ControlCommand> controls(params_.T);
        ...
        for (int t = 0; t < params_.T && !collision; ++t)
        {
            ...
            // ==============================================================
            // COVARIANCE-AWARE ESDF OBSTACLE COST (Option A)
            // d_fused = min(d_esdf, d_lidar)
            // m_eff = m + z * sigma_d (inflated margin)
            // ==============================================================
            float d_esdf = std::numeric_limits<float>::max();
            ...
            {
                d_esdf = esdf_query.distance;
                gradient = esdf_query.gradient;
            }

            float d_lidar = std::sqrt(min_lidar_dist_sq);
            const float d_fused = std::min(d_esdf, d_lidar);

            // Compute covariance-aware margin inflation
            float m_eff = params_.safety_margin;  // Base margin

            // Only inflate when near obstacles (gating)
            if (d_fused < params_.cov_inflation_gate * params_.safety_margin)
            {
                // Compute Jacobian J = ∂p_w/∂ξ for robot center (p_b = [0,0])
                // J = [1, 0, 0]
                //     [0, 1, 0]
                // For center point, only (x,y) uncertainty matters, not theta

                // Extract 2x2 position covariance from 3x3 pose covariance
                Eigen::Matrix2f Sigma_xy = Sigma_pose.block<2,2>(0,0);

                // Compute distance variance: σ²_d = g^T * Σ_xy * g
                float sigma_d_sq = gradient.transpose() * Sigma_xy * gradient;
                float sigma_d = std::sqrt(std::max(0.0f, sigma_d_sq));

                // Clamp sigma_d to avoid over-conservatism
                float sigma_max = params_.cov_sigma_max_clamp * params_.safety_margin;
                sigma_d = std::min(sigma_d, sigma_max);

                // Inflated margin: m_eff = m + z * sigma_d
                m_eff = params_.safety_margin + params_.cov_z_score * sigma_d;
            }

            // Compute penetration with inflated margin
            if (d_fused < m_eff)
            {
                float pen = m_eff - d_fused;
                float pen_normalized = pen / m_eff;
                float obs_c = params_.w_obstacle * pen_normalized * pen_normalized * 100.0f;
                traj_cost += obs_c;
            }
    ...
    // ========================================================================
    // WEIGHT COMPUTATION (same as base version)
    // ========================================================================
    float min_valid_cost = std::numeric_limits<float>::max();
    int num_invalid = 0;
    for (int k = 0; k < params_.K; ++k)
    ...
    for (int k = 0; k < params_.K; ++k)
    {
        weights[k] = std::exp(inv_lambda * (costs[k] - min_valid_cost));
        weight_sum += weights[k];
    }
