...
class PriorPreferences:
    ...
    def __init__(self):
        ...
        self._sigma_py_near = 0.03   # tight — ball about to arrive
        self._sigma_py_far  = 0.10   # loose — ball far or going away
        sigma = np.array([
            0.05,                    # G1: tight  — scoring
            0.20,                    # G2: loose  — ball_y not a direct goal
            self._sigma_py_near,     # G2: py — overridden dynamically
            0.15,                    # G3: moderate — opponent far from ball
        ])
        ...
        self._frames_start = 20   # begin tightening at 20 frames out
    ...
    def contextual_target(self, belief_mu, raw_vel=None):
        """
        ...
        #         (oy - 0.5) * 2.0 * self._max_placement * placement_scale,
        #         -self._max_placement, self._max_placement))
        #     o_star[self.IDX_PY] = float(np.clip(landing_y + offset, 0.0, 1.0))
        # else:
        #     o_star[self.IDX_PY] = landing_y

        # FOCUS ON INTERCEPTION ONLY (GOAL 1)
        o_star[self.IDX_PY] = landing_y
