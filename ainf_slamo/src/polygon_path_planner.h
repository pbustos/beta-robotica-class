...
namespace rc
{
    ...
    class PolygonPathPlanner
    {
        public:
            struct Params
            {
                float robot_radius = 0.25f;          // Minkowski inward offset (m)
        ...
        private:
            ...
            std::vector<Eigen::Vector2f> shrunk_polygon_;   // after Minkowski inward offset

            // ---- visibility graph ----
            struct Edge { int to; float cost; };
            std::vector<std::vector<Edge>> adjacency_;       // precomputed on polygon vertices
            ...
            /// Check if segment [a,b] crosses any edge of the polygon.
